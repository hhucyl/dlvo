...
inline void Domain::UpdateParticlesContacts()
{
    ...
    for(int ip=0; ip<(int)Particles.size(); ++ip)
    {
        ...
        Pa->Fc = 0.0,0.0,0.0;
        int ixs = std::max(std::floor(Pa->X(0) - (1+Pa->eal+Pa->D)*Pa->R - 3*dx),0.0);
        int ixe = std::min(std::ceil(Pa->X(0) + (1+Pa->eal+Pa->D)*Pa->R + 3*dx),(double) nx);
        int iys = std::max(std::floor(Pa->X(1) - (1+Pa->eal+Pa->D)*Pa->R - 3*dx),0.0);
        int iye = std::min(std::ceil(Pa->X(1) + (1+Pa->eal+Pa->D)*Pa->R + 3*dx),(double) ny);
